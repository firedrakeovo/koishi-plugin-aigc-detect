...
const Config = Schema.intersect([
  ...
  Schema.object({
    forward: Schema.boolean().default(false).description('用「合并转发」（聊天记录）展示结果')
      .comment('默认关闭：结果是一条普通消息，会引用（回复）触发它的那条消息，只放结论与明细、不再重复发送原图。打开后改用合并转发——OneBot 的转发消息必须整条只包含转发节点，所以打开时无法带引用；若适配器发不出转发（如 NapCat retcode 1200）会自动降级成普通消息。'),
  }).description('回复样式'),
...
function apply(ctx, config) {
  ...
  ctx
    ...
    .action(async (argv, message) => {
      ...
      // 引用要回复的那条消息：本条消息自带图片 → 引用本条；图片来自被引用消息 → 引用那张原图所在的消息
      const quoteId = ownImages.length ? session.messageId : (quote && quote.id) || session.messageId

      // 普通消息：引用 + 结论 + 明细（不再重复发送原图）
      const sendPlain = () => session.send([
        quoteId ? h('quote', { id: String(quoteId) }) : null,
        h.text(`${summary}\n\n${detailText}`),
      ].filter(Boolean))

      if (config.forward) {
        // 合并转发（聊天记录）：两条纯文本节点（不放原图）。
        // OneBot 的转发要求整条消息只含转发节点，所以这里无法同时带引用。
        // 有些实现/账号发不出转发（NapCat 会回 retcode 1200「发送转发消息失败」，多为 QQ 侧风控），
        // 这时自动降级成普通消息重发（带引用），保证结果一定能看到。
        const head = h('message', {}, h.text(summary))
        const tail = h('message', {}, h.text(detailText))
        try {
          await session.send(h('message', { forward: true }, [head, tail]))
        } catch (e) {
          ctx.logger.warn(`合并转发发送失败，改为普通消息重发：${short(e && e.message, 80)}`)
          await sendPlain()
        }
      } else {
        await sendPlain()
